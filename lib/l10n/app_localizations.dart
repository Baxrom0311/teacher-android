...
extension AppLocalizationsHelpers on AppLocalizations {
  ...
  String assessmentTypeLabelText(String type) {
    switch (type.toLowerCase()) {
      case 'exam':
        return eventTypeExam;
      case 'quiz':
        return _pick(uz: 'Kvizz', ru: 'Квиз', en: 'Quiz');
      case 'test':
        return _pick(uz: 'Test', ru: 'Тест', en: 'Test');
      case 'midterm':
        return _pick(
          uz: 'Oraliq nazorat',
  ...
  String assessmentMaxScoreText(num score) => _pick(
    uz: 'Maksimal ball: $score',
    ru: 'Максимальный балл: $score',
    en: 'Max score: $score',
  );

  String assessmentWeightText(num weight) => _pick(
    uz: 'Og\'irligi: $weight%',
    ru: 'Вес: $weight%',
    en: 'Weight: $weight%',
  );
  ...
  String eventsTypeLabel(String type) {
    switch (type.toLowerCase()) {
      case 'holiday':
        return eventTypeHoliday;
      case 'exam':
        return eventTypeExam;
      case 'meeting':
        return eventTypeMeeting;
      case 'sport':
        return eventTypeSport;
      default:
        return eventTypeOther;
    }
  }

  String gradesQuarterLabel(String quarter) => _pick(
    uz: 'Chorak: $quarter',
    ru: 'Четверть: $quarter',
    en: 'Quarter: $quarter',
  );
  ...
  String dashboardAttendanceRateLabel(num rate) => _pick(
    uz: 'Davomat: ${rate.toStringAsFixed(0)}%',
    ru: 'Посещаемость: ${rate.toStringAsFixed(0)}%',
    en: 'Attendance: ${rate.toStringAsFixed(0)}%',
  );

  String dashboardWeekdayShort(int index) {
    const uz = ['Du', 'Se', 'Cho', 'Pa', 'Ju', 'Sha', 'Ya'];
    const ru = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'];
    const en = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    final normalized = index % 7;
    if (_isUz) return uz[normalized];
    if (_isRu) return ru[normalized];
    return en[normalized];
  }

  String dashboardHeroGreeting(String userName) => _pick(
    uz: '$userName, bugungi ishlaringiz tayyor',
    ru: '$userName, задачи на сегодня готовы',
    en: '$userName, today\'s tasks are ready',
  );

  String dashboardAssessmentMaxScoreLabel(num score) => _pick(
    uz: 'Maks. ball $score',
    ru: 'Макс. балл $score',
    en: 'Max score $score',
  );
  ...
  String dashboardPendingExcusesSubtitle(int count) => _pick(
    uz: '$count ta ko\'rib chiqilmagan ariza',
    ru: '$count необработанных заявок',
    en: '$count pending requests',
  );

  String dashboardOpenConferenceSlotsSubtitle(int count) => _pick(
    uz: '$count ta bo\'sh slot',
  ...
  String homeworkStatusLabel(String status) {
    switch (status.toLowerCase()) {
      ...
      case 'submitted':
        return _pick(uz: 'Yuborilgan', ru: 'Отправлено', en: 'Submitted');
      case 'missing':
        return _pick(uz: 'Topshirilmagan', ru: 'Не сдано', en: 'Missing');
      default:
        return _pick(uz: 'Kutilmoqda', ru: 'Ожидает', en: 'Pending');
    }
  }
  ...
  String homeworkDueDateText(dynamic date) => _pick(
    uz: 'Muddat: ${_formatDate(date)}',
    ru: 'Срок: ${_formatDate(date)}',
    en: 'Due: ${_formatDate(date)}',
  );

  String lessonCoinsSaved(num coins) => _pick(
    uz: '$coins tanga saqlandi',
    ru: 'Сохранено $coins монет',
    en: '$coins coins saved',
  );

  String libraryLoanRecordsCount(int count) => _pick(
    uz: '$count ta ijara qaydi',
    ru: '$count записей выдачи',
    en: '$count loan records',
  );

  String libraryAvailableBooksCount(int count) => _pick(
    uz: '$count ta mavjud kitob',
    ru: '$count доступных книг',
    en: '$count available books',
  );
  ...
  String libraryBorrowedDateLabel(String date) => _pick(
    uz: 'Olingan sana: $date',
    ru: 'Дата выдачи: $date',
    en: 'Borrowed: $date',
  );

  String libraryDueDateLabel(String date) => _pick(
    uz: 'Qaytarish muddati: $date',
    ru: 'Срок возврата: $date',
    en: 'Due date: $date',
  );

  String libraryReturnedDateLabel(String date) => _pick(
    uz: 'Qaytarilgan sana: $date',
    ru: 'Дата возврата: $date',
    en: 'Returned: $date',
  );

  String libraryCopiesLabel(int availableCopies, [int? totalCopies]) {
    if (totalCopies == null) {
      return _pick(
        uz: 'Nusxalar: $availableCopies',
        ru: 'Экземпляры: $availableCopies',
        en: 'Copies: $availableCopies',
      );
    }

    return _pick(
      uz: 'Nusxalar: $availableCopies/$totalCopies',
      ru: 'Экземпляры: $availableCopies/$totalCopies',
      en: 'Copies: $availableCopies/$totalCopies',
    );
  }
  ...
  String mealTypeLabel(String value) {
    switch (value.toLowerCase()) {
      ...
      case 'snack':
        return _pick(uz: 'Tamaddi', ru: 'Перекус', en: 'Snack');
      default:
        return value;
    }
  }
  ...
  String paymentPeriodLabel({
    ...
    final month = periodMonth?.toString() ?? '-';
    final year = periodYear?.toString() ?? '-';
    final type =
        payType?.toString() ??
        _pick(uz: 'To\'lov', ru: 'Платеж', en: 'Payment');
    final method = paymentMethod?.toString();
    final parts = <String>['$type', '$month/$year'];
  ...
  String academicYearLabel(String value) => _pick(
    uz: 'O\'quv yili: $value',
    ru: 'Учебный год: $value',
    en: 'Academic year: $value',
  );

  String groupLabelText(String value) =>
      _pick(uz: 'Guruh: $value', ru: 'Группа: $value', en: 'Group: $value');

  String roomLabelText(String value) =>
      _pick(uz: 'Xona: $value', ru: 'Кабинет: $value', en: 'Room: $value');
}
