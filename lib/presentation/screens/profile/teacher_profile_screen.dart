...
class _TeacherProfileScreenState extends ConsumerState<TeacherProfileScreen> {
        ...
        child: profileAsync.when(
          data: (profile) {
            ...
            final displayTeacherName =
                teacher.name.isNotEmpty ? teacher.name : l10n.teacherFallbackName;
                      ...
                      delegate: SliverChildListDelegate([
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            if (profile.canEdit)
                              AnimatedPressable(
                                onTap: () => context.push('/profile/works/create'),
                                child: Text(
                                  l10n.addAction,
                                  style: TextStyle(
                        ...
                        AnimatedPressable(
                          ...
                          child: PremiumCard(
                            color: TeacherAppColors.error.withValues(alpha: 0.05),
                            border: Border.all(
                              color: TeacherAppColors.error.withValues(alpha: 0.2),
                            ),
                            child: Row(
  ...
  Widget _buildSettingsItem(
    ...
    return ListTile(
      ...
      subtitle: Text(
        subtitle,
        style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: colorScheme.onSurface.withValues(alpha: 0.4)),
      ),
      trailing: Icon(
  ...
  void _showLogoutDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(l10n.logoutConfirmMessage, style: const TextStyle(fontWeight: FontWeight.w600)),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            child: Text(l10n.logoutAction, style: const TextStyle(fontWeight: FontWeight.w900)),
          ),
        ],
      ),
    );
  ...
  String _getThemeName(BuildContext context, ThemeMode mode) {
    final l10n = context.l10n;
    switch (mode) {
      case ThemeMode.system: return l10n.themeSystem;
      case ThemeMode.light: return l10n.themeLight;
      case ThemeMode.dark: return l10n.themeDark;
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text(l10n.changeLanguage, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            ),
            _LanguageTile(label: l10n.langUz, code: 'uz', isSelected: ref.read(appLocaleProvider) == AppLocale.uz, onTap: () { ref.read(appLocaleProvider.notifier).setLocale(AppLocale.uz); Navigator.pop(context); }),
            _LanguageTile(label: l10n.langRu, code: 'ru', isSelected: ref.read(appLocaleProvider) == AppLocale.ru, onTap: () { ref.read(appLocaleProvider.notifier).setLocale(AppLocale.ru); Navigator.pop(context); }),
            _LanguageTile(label: l10n.langEn, code: 'en', isSelected: ref.read(appLocaleProvider) == AppLocale.en, onTap: () { ref.read(appLocaleProvider.notifier).setLocale(AppLocale.en); Navigator.pop(context); }),
            const SizedBox(height: 24),
          ],
        ),
      ),
      ...
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            Padding(
              padding: const EdgeInsets.all(24.0),
              child: Text(l10n.changeTheme, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            ),
            _ThemeTile(label: l10n.themeSystem, icon: Icons.brightness_auto_rounded, isSelected: ref.read(appThemeModeProvider) == ThemeMode.system, onTap: () { ref.read(appThemeModeProvider.notifier).setThemeMode(ThemeMode.system); Navigator.pop(context); }),
            _ThemeTile(label: l10n.themeLight, icon: Icons.light_mode_rounded, isSelected: ref.read(appThemeModeProvider) == ThemeMode.light, onTap: () { ref.read(appThemeModeProvider.notifier).setThemeMode(ThemeMode.light); Navigator.pop(context); }),
            _ThemeTile(label: l10n.themeDark, icon: Icons.dark_mode_rounded, isSelected: ref.read(appThemeModeProvider) == ThemeMode.dark, onTap: () { ref.read(appThemeModeProvider.notifier).setThemeMode(ThemeMode.dark); Navigator.pop(context); }),
            const SizedBox(height: 24),
          ],
        ),
      ),
      ...
      child: Column(
        children: [
          ...
          Text(
            ...
            style: TextStyle(color: Colors.white.withValues(alpha: 0.6), fontSize: 14, fontWeight: FontWeight.w700),
          ),
...
class _AvatarCircle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final initials = displayTeacherName.isNotEmpty ? displayTeacherName[0].toUpperCase() : 'T';
    return Container(
      width: 100,
      height: 100,
      decoration: BoxDecoration(
        ...
        children: [
          Row(
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.description_rounded, color: colorScheme.primary, size: 24),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      work.title.isNotEmpty ? work.title : 'Scientific Work',
              ...
              if (canEdit && work.createdBy == teacherId)
                IconButton(
                  onPressed: onDelete,
                  icon: const Icon(Icons.delete_outline_rounded, color: TeacherAppColors.late, size: 20),
                ),
            ],
          ),
          if (work.authors.isNotEmpty) ...[
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: work.authors.map((author) => _AuthorChip(name: author.name)).toList(),
            ),
          ],
        ],
      ),
...
class _LanguageTile extends StatelessWidget {
  ...
  const _LanguageTile({required this.label, required this.code, required this.isSelected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: Container(
        ...
        child: Text(code.toUpperCase(), style: TextStyle(color: colorScheme.primary, fontSize: 12, fontWeight: FontWeight.w900)),
      ),
...
class _ThemeTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      onTap: onTap,
      title: Text(label, style: const TextStyle(fontWeight: FontWeight.w700)),
      leading: Icon(icon, color: isSelected ? colorScheme.primary : colorScheme.onSurfaceVariant.withValues(alpha: 0.5)),
      trailing: isSelected ? Icon(Icons.check_circle_rounded, color: colorScheme.primary) : null,
    );
  }
}
